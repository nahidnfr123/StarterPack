        ...
        <div class="flex items-center justify-between py-6 md:justify-start md:space-x-10">
          ...
          <PopoverGroup as="nav" class="hidden space-x-10 md:flex">
            <template v-for="(nv, n) in navigationLinks" :key="n">
              <RouterLink :to="nv.link" exact-active-class="text-indigo-600" class="text-base text-lg font-medium text-black hover:text-gray-900 duration-300">{{ nv.title }}</RouterLink>
            </template>
          </PopoverGroup>
          ...
          <div class="hidden items-center justify-end md:flex md:flex-1 lg:w-0">
            ...
            <div v-else>
              <RouterLink v-if="!hideLoginButton" to="/auth/login" class="text-sm font-semibold leading-6 text-gray-900 px-4 py-3 rounded-xl hover:bg-gray-100 duration-500">
