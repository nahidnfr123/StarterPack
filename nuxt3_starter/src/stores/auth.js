...
import {getData, submitData} from "~/composables/useRequest";
...
export const useAuthStore = defineStore('auth', {
    state: () => ({
        isLoggedIn: false,
        token: getToken() || null,
        user: process.client ? (JSON.parse(getUser()) || null) : null
    }),
    getters: {},
    actions: {
        async register(payload) {
            const {data, pending, error, refresh} = await submitData('register', payload);
        ...
        async login(payload) {
            const {data, pending, error, refresh} = await submitData('login', payload);
